...
def extract_issue_data(raw_issue):
    issue = {
        "repo": raw_issue[1]['repository_url'].split('repos/')[1],
        "language": raw_issue[0],
        "title": raw_issue[1]['title'],
        "url": raw_issue[1]['html_url'],
        "comments": raw_issue[1]['comments'],
    }
...
async def extract_repos(user, session, repos_per_page=100):
    ...
    user_url = f"https://api.github.com/users/{user}/repos?page=1&per_page={repos_per_page}"
    async with session.get(user_url) as resp:
